...
def test_login_post(client, create_user, test_password):
    user = create_user()
...
def test_login_post_wrong_password(client, create_user):
    user = create_user()
...
def test_signup_post(client, django_user_model, test_password):
...
@pytest.mark.django_db(transaction=True)
def test_signup_post_invalid_form(client, django_user_model):
    username = str(uuid.uuid4())
